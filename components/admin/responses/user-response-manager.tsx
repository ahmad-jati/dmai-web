...
import { ArrowLeftIcon, MagnifyingGlassIcon, ArrowSquareOutIcon } from "@phosphor-icons/react"
import { fmtLocalTime, fmtDuration, groupByDay, fmtClock } from "@/lib/session-helper"
import type { RecentCompletion } from "@/lib/session-helper"
...
function PageSkeleton() {
  return (
    <div className="flex flex-col gap-4 animate-pulse">
      ...
      <div className="h-8 w-64 bg-muted/50 rounded-sm" />
...
function ResultsPagination({
  ...
  page: number; totalPages: number; onPageChange: (p: number) => void; total: number; pageSize: number
}) {
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <PaginationItem key={p}>
              <Button
                ...
                size="sm"
                className="w-8 h-8 p-0 rounded-sm font-medium text-sm hover:bg-foreground/80"
...
export function UserResponseManager() {
  const router = useRouter()
  const cache = useRef<RecentCompletion[] | null>(null)
  ...
  useEffect(() => {
    if (cache.current) {
      setRecentCompletions(cache.current)
      setLoading(false)
      return
    }
  ...
  const filtered = recentCompletions.filter((c) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold">Track riwayat user</h2>
          <p className="text-sm text-muted-foreground mt-0.5">{filtered.length} riwayat</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-muted/40 border-b border-border">
                ...
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-muted-foreground uppercase tracking-wide w-[18%] hidden sm:table-cell">Waktu Mulai</th>
                    ...
                    >
                      <td className="px-4 py-3">
                        <p className="text-sm truncate text-foreground/80">
                          {(page - 1) * PAGE_SIZE + paginated.findIndex((item) => item.id === c.id) + 1}
                        </p>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          <div className="w-7 h-7 rounded-full bg-foreground/90 flex items-center justify-center text-2xs font-bold text-background shrink-0">
                      ...
                      <td className="px-4 py-3 hidden sm:table-cell">
                        <p className="text-xs text-muted-foreground tabular-nums whitespace-nowrap">{fmtClock(c.started_at)}</p>
                      </td>
                      <td className="px-4 py-3 hidden sm:table-cell">
                        <p className="text-xs text-muted-foreground tabular-nums whitespace-nowrap">{fmtClock(c.completed_at)}</p>
                      </td>
