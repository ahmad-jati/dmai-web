...
import { ArrowUpRightIcon, PersonSimpleTaiChiIcon, TimerIcon, LockSimpleIcon } from "@phosphor-icons/react/dist/ssr"
...
async function OtherSessionListServer({ excludeSlug }: { excludeSlug?: string }) {
  ...
  return (
    <div className="grid sm:grid-cols-3 grid-cols-1 gap-3.5 w-full">
      {sorted.map((session) => {
        const cardContent = (
          <>
            <div className="relative md:w-full md:h-40 2xs:w-34 2xs:h-30 w-24 h-20 2md:rounded-[14px] rounded-[10px] overflow-hidden">
              <Image
            ...
            <div className="flex flex-col items-start xs:gap-1.5 gap-2 2md:px-1 w-full">
              <div className="flex items-center w-full gap-2">
                <p
                  className={`text-p/5 max-w-140 font-semibold 2md:text-left text-left ${
                    session.is_locked
        ...
        return session.is_locked ? (
          <div
            key={session.slug}
            className="
              flex flex-col items-end 2xs:gap-4 gap-2
              bg-background dark:bg-card 2md:rounded-[20px] rounded-lg border border-foreground/10 w-full overflow-hidden
